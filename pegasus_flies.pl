...
for($i = 1; $i <= $#ARGV; ++$i){
	...
	if(length($field)>1){
		...
		if($field eq "-chr"){ # Do test for specific chromosome
			...
			print "-chr $dochr\n";
			if ($dochr > 9 || $dochr < 1){
				die "Error: -chr must be a number between 1 and 9\n"; 
			}
		}
...
# $time_stamp = $dir/$DayOfYear.$Minute.$Second.$dochr;

$dir_chir = $dir."/".$outfile;
print $dir_chir;
mkdir "$dir_chir", 0777 or die "Error: cannot make working directory. Check permission settings: $!";

# print "\nCreating working directory: $dir_chir\n";
...
chdir "$dir_chir" or die "Error: cannot change to working directory: $!";
...
read.table('$chip/glist-dm5.57',colClasses=c('character',rep('integer',2),'character')) -> startstop
...
#		print "--hapsims function exit--\n";
}

sub sort_by_number{ $b <=> $a }

sub plink{ # Use plink to generate ld matrix, then check for errors
#	print "Create LD matrix...";
	if(defined($custom)){
#		print "custom\n";
		system("plink --bfile custom$chr --extract tempgene.snp --matrix --r --no-sex --out ld > /dev/null");
	}
	elsif(!defined($ldfile)){
#		print "not defined?\n";
		system("plink --bfile $chip/genebeds/$gene --extract tempgene.snp --matrix --r --no-sex --out ld > /dev/null");
	}
#	print "LD matrix created\n";
	...
	close PRINTPLINKLD;
	close PLINKLD;
#	print "closing LD loop\n\n";
}

sub customtest{ # using custom set of SNPs - default
	print "\nReading custom genotypes...done\n\n";
	open(HGLIST,"../glist-dm5.57") or die "Error: Cannot find glist-dm5.57 [1]\n";
	@hglist = <HGLIST>;
	close(HGLIST);
...
#		if ($chr=='X') {$chr =6;} 
		...
		foreach $gene (@allgenes){
			...
			chomp($gene);
			system("grep -w \"$gene\" ../glist-dm5.57 > gene.position");
			...
			system("plink --bfile custom$chr --chr $chr --from-bp $start --to-bp $stop --write-snplist --noweb --silent > /dev/null");  ## Original
# 			system("plink --bfile custom$chr --chr $chr --from-bp $start --to-bp $stop --force-intersect --write-snplist --noweb --silent > /dev/null");
			if(-e "plink.snplist"){
...
sub docustomchr { # using custom set of SNPs - chromosome
	print "\nReading custom genotypes, by chromosome...done\n\n";
	
	open(HGLIST,"../glist-dm5.57") or die "Error: Cannot find glist-dm5.57 [2]\n";
	@hglist = <HGLIST>;
	#close(HGLIST);
...
sub ldfiletest{ # using custom LD file - default
	open(HGLIST,"../glist-dm5.57") or die "Error: Cannot find glist-dm5.57 [3]\n";
	@hglist = <HGLIST>;
	close(HGLIST);

	for ($chr = 1; $chr <= 8; $chr++) {	 
		system("awk '\$1==$chr {print \$0}' ../glist-dm5.57 > allgene$chr\n"); ## find all genes on chromosome
#		if ($chr==6) {$chr =='X';} 
...
sub doldchr{ # using custom LD file - for one chromosome
	open(HGLIST,"../glist-dm5.57") or die "Error: Cannot find glist-dm5.57[4]\n";
	@hglist = <HGLIST>;
	close(HGLIST);
...
#	print "--maketempgener loop begin--\n";
	system("
...
tempgene <- unique(tempgene) 
#tempgene_snp <-unique(tempgene[1,]) 
write.table(tempgene,'tempgene',row.names=F,col.names=F,quote=F)

EOF");

system("awk '{print \$1;}' tempgene | awk '!seen[\$0]++' > tempgene.snp"); 
#print "--maketempgener loop exit--\n";
}
}
